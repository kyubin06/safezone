...
// 이 방식은 실제 도로 데이터에 의존하지 않고 항상 확실하게 서로 다른 모양의 후보를 만든다.
function bulgeRoute(basePoly,baseLen,startFrac,endFrac,sideSign,bulgeM){
  const cum=[0];
  for(let i=0;i<basePoly.length-1;i++)cum.push(cum[i]+segDist(basePoly[i],basePoly[i+1]));
  ...
  return basePoly.map((p,i)=>{
    ...
    const bump=Math.sin(Math.PI*t)*bulgeM; // 0→최대→0, 부드럽게 이어지므로 유턴이 생기지 않음
    const p0=basePoly[Math.max(0,i-1)],p1=basePoly[Math.min(basePoly.length-1,i+1)];
    const tx=mx(p1.lng)-mx(p0.lng),ty=my(p1.lat)-my(p0.lat);
    const tl=Math.hypot(tx,ty)||1;
    const nx=-ty/tl,ny=tx/tl; // 그 지점에서의 진행방향에 수직인 방향(미터 단위)
...
async function fetchRoutes(){
  ...
  // 실제 도로망 상황과 무관하게 항상 서로 뚜렷이 다른 모양이 나옴을 보장한다.
  const windows=[[0.08,0.4],[0.32,0.64],[0.56,0.9]];
  const mags=[110,210];
  windows.forEach(([a,b])=>{
    [1,-1].forEach(sign=>{
      mags.forEach(m=>{
        const poly=bulgeRoute(base.poly,base.len,a,b,sign,m);
  ...
  return uniq; // 유턴 없는 후보 전부 반환 (보통 8~13개 정도) — 최종 화면 노출은 점수 상위 2~3개만
}
