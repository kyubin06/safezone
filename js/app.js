...
function scoreRoutesFromMetrics(metricsList){
  ...
  return metricsList.map((m,i)=>{
    ...
    let score=Math.round(38+56*cover+Math.min(6,m.zoneN*2));
...
async function fetchRoutes(){
  ...
  // 우회 경유지 오프셋을 경로 거리에 비례해 잡아야 실제로 다른 골목/도로로 우회한 경로가 나옴
  // (고정된 작은 오프셋은 원래 경로와 길이·형태가 거의 같아져서 "다른 경로"라고 부를 수 없었음)
  const off=Math.max(0.0045,(baseLen/111000)*0.22);
  async function viaRoute(sign){
    const via={lat:midLat+(-dx/ln)*off*sign,lng:midLng+(dy/ln)*off*sign};
    try{
      const r=await fetch(osrmUrl([s,via,d]));const j=await r.json();
      if(j.code==='Ok'&&j.routes[0])return {poly:decodeGeo(j.routes[0]),len:j.routes[0].distance};
    }catch(e){}
    return null;
  }
  // 최소 2~3개의 후보가 나오도록 좌/우 양쪽으로 우회 경로를 시도
  if(routes.length<2){const r=await viaRoute(1);if(r)routes.push(r);}
  if(routes.length<3){const r=await viaRoute(-1);if(r)routes.push(r);}
