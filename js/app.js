...
function scoreRoutesFromMetrics(metricsList){
  ...
  // 보안등은 좌표 데이터가 없어 경로별 위치로 구분할 수 없다(밀도가 사실상 상수라 슬라이더가 죽음).
  // → "얼마나 곧장(짧게) 가는가"로 대신 반영한다. 보안등 비중을 높이면 불필요하게 돌지 않는
  //   가장 직선적인 경로를 선호하게 되어 슬라이더가 실제로 순위를 바꾼다.
  const nG=normList(metricsList.map(m=>minLen/m.len));
  const W=getWeights();
  const covers=metricsList.map((m,i)=>W.c*nC[i]+W.g*nG[i]+W.b*nB[i]);
  return metricsList.map((m,i)=>{
    const cover=covers[i];
    // Safe-Point 점수는 58~96 범위 안에서만 나오도록 제한(표시용)
    let score=Math.round(62+30*cover+Math.min(6,m.zoneN*2));
...
async function fetchRoutes(){
  ...
  // 1) OSRM 실제 보행 경로: 직선 최단 + 대안. 유턴성 경로는 제외.
  let base=[];
  try{
    const res=await fetch(osrmUrl([s,d]));const js=await res.json();
    if(js.code==='Ok'){js.routes.slice(0,3).forEach(rt=>{const poly=decodeGeo(rt);if(!hasUTurn(poly))base.push({poly,len:rt.distance});});}
  }catch(e){}
  const baseLen=base.length?base[0].len:segDist(s,d)*1.3;
  const dx=d.lng-s.lng,dy=d.lat-s.lat,ln=Math.hypot(dx,dy)||1;
  // 자연스러운 "옆 골목/도로" 수준의 우회만 허용한다.
  // (오프셋이 너무 크면 유턴·불필요한 우회가 생기고, 너무 작으면 같은 길로 합류해 후보가 안 갈라짐)
  const baseOff=Math.max(0.003,(baseLen/111000)*0.20); // 대략 330m 안팎
  ...
  // 중간 지점 1곳만 경유해 완만하게 갈라지게 한다(지그재그·유턴 방지).
  // 유턴이거나 1.4배 넘게 돌아가면 버리고, 오프셋을 줄여 다시 시도한다.
    ...
    for(const off of [baseOff,baseOff*0.6,baseOff*0.35]){
      ...
      try{
        ...
        if(j.code==='Ok'&&j.routes[0]){
          ...
          if(len>baseLen*1.4)continue;   // 1.4배 넘게 돌아가는(굳이 찍고 가는) 경로는 배제
  ...
  // 후보를 세 가지 성격으로 분명히 나눠 가중치가 실제로 다른 경로를 고르게 한다.
  //  · 직선 최단  → 보안등(곧장 가기) 대표
  //  · CCTV 밀집 방향 → CCTV 대표
  //  · 비상벨 방향   → 비상벨 대표 (CCTV와 다른 쪽으로 갈라지게)
  const routes=[];
  if(base[0])routes.push(base[0]);
  const cctvR=await viaRoute(sCctv);if(cctvR)routes.push(cctvR);
  let bSign=sBell;if(bSign===sCctv)bSign=-bSign;
  const bellR=await viaRoute(bSign);if(bellR)routes.push(bellR);
  // 후보가 부족하면 OSRM 대안 → 반대쪽 우회 순으로 보충
  for(let i=1;i<base.length&&routes.length<3;i++)routes.push(base[i]);
