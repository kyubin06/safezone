...
async function fetchRoutes(){
  ...
  const jobs=[];
  fracs.forEach(f=>{jobs.push(viaRoute(f,1));jobs.push(viaRoute(f,-1));});
  const extra=(await Promise.all(jobs)).filter(Boolean);
  ...
  return uniq; // 유턴 없는 후보 전부 반환 (보통 8~12개 정도) — 최종 화면 노출은 점수 상위 2~3개만
}
