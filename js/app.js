...
async function fetchRoutes(){
  ...
  // 우회 경유지 오프셋을 경로 거리에 비례해 잡되, 너무 크게 잡으면 굳이 필요없는 먼 도로까지
  // 돌아가는 부자연스러운 경로가 나오므로 예전보다 낮춰서(0.22→0.16) 과한 우회를 줄임
  const baseOff=Math.max(0.0035,(baseLen/111000)*0.16);
  const corridorDeg=0.0032; // 직선 경로 기준 대략 350m 이내만 "근처"로 봄
  ...
  // → 가중치를 바꾸면 순서·점수만 바뀌는 게 아니라 실제로 다른 물리적 경로가 나오게 됨
  ...
  async function viaRoute(frac,sign){
    ...
    // 오프셋이 너무 크면 안전요소를 더 지나려고 갔다가 되돌아오는 "유턴" 형태이거나
    // 원래 경로보다 훨씬 길어지는 불필요한 우회가 나올 수 있어, 그런 경우 오프셋을 줄여 재시도한다.
    for(const off of [baseOff,baseOff*0.6,baseOff*0.35]){
      ...
      try{
        ...
        if(j.code==='Ok'&&j.routes[0]){
          ...
          if(len>baseLen*1.35)continue; // 원래 경로보다 35% 이상 길어지면 불필요한 우회로 보고 제외
  ...
  // 후보1: 경로 앞쪽(40%)에서, 현재 가중치가 더 높은 요소가 몰린 방향으로 우회
  if(routes.length<3){const r=await viaRoute(0.4,priority[0].sign);if(r)routes.push(r);}
  // 후보2: 경로 뒤쪽(65%)에서, 두 번째 요소가 몰린 방향으로 우회 (같은 방향이면 반대쪽으로 강제 분리)
  if(routes.length<3){let sign2=priority[1].sign;if(sign2===priority[0].sign)sign2=-sign2;const r=await viaRoute(0.65,sign2);if(r)routes.push(r);}
  // 그래도 후보가 부족하면 중간지점 양쪽 우회로 보충
  if(routes.length<2){const r=await viaRoute(0.5,1);if(r)routes.push(r);}
  if(routes.length<3){const r=await viaRoute(0.5,-1);if(r)routes.push(r);}
