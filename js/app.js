...
async function fetchRoutes(){
  ...
  const baseOff=Math.max(0.0045,(baseLen/111000)*0.22);
  async function viaRoute(sign){
    // 오프셋이 너무 크면 안전요소를 더 지나려고 갔다가 되돌아오는 "유턴" 형태의 경로가 나올 수 있어,
    // 유턴이 감지되면 오프셋을 줄여가며 자연스러운 우회 경로가 나올 때까지 다시 시도한다.
    for(const off of [baseOff,baseOff*0.6,baseOff*0.35]){
      ...
      try{
        ...
        if(j.code==='Ok'&&j.routes[0]){
          const poly=decodeGeo(j.routes[0]);
          if(!hasUTurn(poly))return {poly,len:j.routes[0].distance};
        }
      }catch(e){}
    }
    return null;
  }
  // 최소 2~3개의 후보가 나오도록 좌/우 양쪽으로 우회 경로를 시도 (유턴하는 경로는 제외)
