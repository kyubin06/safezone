...
function scoreRoute(poly,len){
  ...
  const W=getWeights();
  const cover=W.c*sat(dC,1.6)+W.g*sat(dG,2.4)+W.b*sat(dB,0.9);
  let score=Math.round(38+56*cover+Math.min(6,zoneN*2));
  score=Math.max(30,Math.min(99,score));
  return {score,len,f,counts:{cctv:f.cctv.length,guard:guardN,bell:f.bell.length,zone:zoneN}};
}
...
function initSetupMap(){
  ...
  L.tileLayer('https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png',{
    maxZoom:19,subdomains:'abcd',attribution:'© OpenStreetMap © CARTO'}).addTo(setupMap);
...
async function goAnalyze(){
  ...
  try{
    ...
    state.routes=routes.map((rt,i)=>{const poly=rt.poly,sc=scoreRoute(poly,rt.len);return {key:'r'+i,label:i===0?'추천 경로 후보 A':'경로 후보 '+String.fromCharCode(65+i),poly,dist:rt.len,...sc};});
...
async function fetchRoutes(){
  ...
  let routes=[];
  if(js.code==='Ok'){js.routes.slice(0,3).forEach(rt=>routes.push({poly:decodeGeo(rt),len:rt.distance}));}
  // ensure at least 2 candidates: add a via-detour route
  if(routes.length<2){
    const midLat=(s.lat+d.lat)/2,midLng=(s.lng+d.lng)/2;
    const dx=d.lng-s.lng,dy=d.lat-s.lat,ln=Math.hypot(dx,dy)||1;
    const off=0.006;const via={lat:midLat+(-dx/ln)*off,lng:midLng+(dy/ln)*off};
    try{const r2=await fetch(osrmUrl([s,via,d]));const j2=await r2.json();
      if(j2.code==='Ok'&&j2.routes[0])routes.push({poly:decodeGeo(j2.routes[0]),len:j2.routes[0].distance});}catch(e){}
  }
  // dedupe by length similarity
  const uniq=[];routes.forEach(r=>{if(!uniq.some(u=>Math.abs(u.len-r.len)<40))uniq.push(r);});
