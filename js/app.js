...
// ===== Safe-Point = 최종 안심 점수(54~96). 60 미만이면 '탈락'. =====
// 경로 안전도(가장 큼) + 안심미션 수행 + 통과한 안전요소 밀도를 합쳐 산정.
//  → 안전한 추천 경로 + 미션 수행이면 성공권, 낮은 경로를 고르고 미션까지 건너뛰면 탈락 가능.
function computeSafePoint(w){
  const routeQ=Math.max(0,Math.min(1,(state.chosen.score-57)/40));       // 경로 Safe-Score(57~97) → 0~1
  const missionRate=w.missionTotal>0 ? w.missionsDone/w.missionTotal : 1; // 미션 수행률 0~1
  const passTotal=w.pass.cctv+w.pass.guard+w.pass.bell+w.pass.zone;
  const density=Math.min(1, passTotal/Math.max(1, w.len/120));           // 통과 안전요소 밀도 0~1
  const q=0.60*routeQ + 0.25*missionRate + 0.15*density;                 // 0~1
  return Math.max(54, Math.min(96, Math.round(54 + 42*q)));
}
function startWalk(){
  ...
  state.walk.points=computeSafePoint(state.walk);pointVal.textContent=state.walk.points;
...
function showToast(m,title,desc,pts){const layer=document.getElementById('toastLayer');
  ...
  el.innerHTML=`<div class="tic" style="background:${m.bg}">${m.ic}</div><div class="tt"><b>${title}</b><p>${desc}</p><span class="pts">안심도 +${pts}</span></div>`;
...
function resumeMissionWalk(res){
  ...
  if(res&&res.success){
    // 미션 성공 → 수행률 반영으로 Safe-Point 상승 (SAFE 게이지는 상승하지 않음)
    w.missionsDone++;w.missionBonus++;
    w.points=computeSafePoint(w);animNum('pointVal',w.points);
  }
  w.missionActive=false;w.paused=false;w.last=null;
}
function finishWalk(){const w=state.walk;stopWalk();w.gauge=w.target;setGauge(w.target);w.points=computeSafePoint(w);setTimeout(showResult,650);}
...
function showResult(){const w=state.walk;show('result');const sc=state.chosen.score;
  ...
  const passed=w.points>=60; // Safe-Point 60 미만이면 탈락
  ...
  document.getElementById('resNote').innerHTML=`Safe-Point는 경로 안전도(Safe-Score ${sc}점)와 안심미션 수행을 합산한 최종 점수예요. <b>60점 이상이면 성공</b>, 미만이면 탈락입니다.<br>기존 신고 중심 서비스와 달리, Safe-Point AI는 이동 전에 미리 안전한 길을 안내하고 지속적인 이용 동기를 제공합니다.`;
  fireConfetti(passed);}
