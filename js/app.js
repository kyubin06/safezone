...
async function goAnalyze(){
  ...
  try{
    ...
    // 유턴 없는 후보(보통 8~12개)를 전부 채점한 뒤, 점수가 가장 높은 상위 2~3개만 화면에 추천으로 노출
    const metricsList=routes.map(rt=>routeRawMetrics(rt.poly,rt.len));
    const scoredAll=scoreRoutesFromMetrics(metricsList);
    const combined=routes.map((rt,i)=>({poly:rt.poly,dist:rt.len,...scoredAll[i]}));
    combined.sort((a,b)=>b.score-a.score);
    const TOP_N=Math.min(3,combined.length);
    state.routes=combined.slice(0,TOP_N).map((r,i)=>({key:'r'+i,label:i===0?'가장 안전한 경로':'대체 경로 '+i,...r}));
...
// 폴리라인에서 특정 누적거리 s에 해당하는 인덱스를 찾는다 (cum: 누적거리 배열)
function idxAtS(cum,s){for(let i=0;i<cum.length;i++)if(cum[i]>=s)return i;return cum.length-1;}
// 지정된 인덱스 구간에서만 누적 회전각(도)을 구한다.
// 원래 경로가 공원 등에서 자연스럽게 구불구불한 부분까지 포함해서 계산하면 오탐이 나므로,
// 반드시 "우리가 실제로 손댄 구간"만 국소적으로 검사해야 함
function localTurningDeg(poly,iStart,iEnd){
  let total=0,prevAngle=null;
  for(let i=Math.max(0,iStart);i<Math.min(poly.length-1,iEnd);i++){
    const a=poly[i],b=poly[i+1];
    if(segDist(a,b)<1)continue;
    const ang=Math.atan2(my(b.lat)-my(a.lat),mx(b.lng)-mx(a.lng));
    if(prevAngle!==null){
      let dlt=ang-prevAngle;
...
// 우회를 적용할 구간이 원래 경로에서부터 이미 직선에서 많이 벗어나 있는지(=이미 구불구불한지) 측정.
// 이미 구불구불한 구간(예: 공원·주차장 내부 산책로)에 추가로 밀어내면 매듭처럼 꼬이기 쉬우므로
// 그런 구간은 애초에 우회 후보 생성 대상에서 제외한다.
function chordDeviation(basePoly,cum,s0,s1){
  const pStart=posAtS(basePoly,s0),pEnd=posAtS(basePoly,s1);
  const ax=mx(pStart.lng),ay=my(pStart.lat),bx=mx(pEnd.lng),by=my(pEnd.lat);
  const dx=bx-ax,dy=by-ay,L2=dx*dx+dy*dy||1;
  let maxDev=0;
  for(let i=0;i<basePoly.length;i++){
    const s=cum[i];
    if(s<s0||s>s1)continue;
    const px=mx(basePoly[i].lng),py=my(basePoly[i].lat);
    const t=Math.max(0,Math.min(1,((px-ax)*dx+(py-ay)*dy)/L2));
    const cx=ax+t*dx,cy=ay+t*dy;
    maxDev=Math.max(maxDev,Math.hypot(px-cx,py-cy));
  }
  return maxDev;
}
// 기본 경로(basePoly)의 일부 구간을 자연스러운 곡선으로 옆으로 밀어서
// "다른 골목으로 우회한 것처럼 보이는" 새 경로를 계산으로 만든다.
// OSRM에 다시 요청해서 대체 경로를 "기대"하는 방식은, 실제 보행자 도로망이 성긴 지역에서는
// via 지점을 아무리 바꿔도 같은 큰길로 되돌아와 버려서 매번 거의 똑같은 경로만 나오는 문제가 있었다.
// 구간 전체에 "하나의 고정된 방향"으로만 밀어야 경로가 스스로 꼬이지 않는다.
// (매 점마다 그 지점의 국소 진행방향으로 다시 수직 계산하면, 원래 경로가 구불구불할 때
//  밀어내는 방향도 같이 급격히 회전해서 매듭처럼 꼬이는 경로가 나올 수 있음 — 실제로 겪은 버그)
function bulgeRoute(basePoly,cum,baseLen,startFrac,endFrac,sideSign,bulgeM){
  const s0=baseLen*startFrac,s1=baseLen*endFrac;
  const pStart=posAtS(basePoly,s0),pEnd=posAtS(basePoly,s1);
  let tx=mx(pEnd.lng)-mx(pStart.lng),ty=my(pEnd.lat)-my(pStart.lat);
  let tl=Math.hypot(tx,ty);
  if(tl<5){const gs=basePoly[0],ge=basePoly[basePoly.length-1];
    tx=mx(ge.lng)-mx(gs.lng);ty=my(ge.lat)-my(gs.lat);tl=Math.hypot(tx,ty)||1;}
  const cosLat=Math.cos(LAT0*RAD);
  const nx=-ty/tl,ny=tx/tl; // 구간 전체에 고정된 수직 방향
  return basePoly.map((p,i)=>{
    const s=cum[i];
    if(s<s0||s>s1)return p;
    const t=(s-s0)/((s1-s0)||1);
    const bump=Math.sin(Math.PI*t)*bulgeM; // 0→최대→0, 부드럽게 이어짐
    const offXm=nx*bump*sideSign,offYm=ny*bump*sideSign;
    return {lat:p.lat+offYm/MPD,lng:p.lng+offXm/(cosLat*MPD)};
  });
}
async function fetchRoutes(){
  ...
  let baseRoutes=[];
  if(js.code==='Ok'){js.routes.slice(0,3).forEach(rt=>{const poly=decodeGeo(rt);if(!hasUTurn(poly))baseRoutes.push({poly,len:rt.distance});});}
  if(!baseRoutes.length)return [];
  const base=baseRoutes[0];
  const cum=[0];for(let i=0;i<base.poly.length-1;i++)cum.push(cum[i]+segDist(base.poly[i],base.poly[i+1]));
  let routes=[...baseRoutes];
  // 경로 구간(앞/중간/뒤) × 좌우 × 우회 폭(작게/크게) 조합으로 다양한 후보를 만든다.
  const windows=[[0.08,0.4],[0.32,0.64],[0.56,0.9]];
  const mags=[110,210];
  windows.forEach(([a,b])=>{
    // 이미 구불구불한 구간(공원·주차장 산책로 등)은 우회 적용 대상에서 제외 — 매듭 방지
    if(chordDeviation(base.poly,cum,base.len*a,base.len*b)>50)return;
    const iStart=idxAtS(cum,base.len*a),iEnd=idxAtS(cum,base.len*b);
    [1,-1].forEach(sign=>{
      mags.forEach(m=>{
        const poly=bulgeRoute(base.poly,cum,base.len,a,b,sign,m);
        if(hasUTurn(poly))return;
        // 손댄 구간에서 진행방향이 한 바퀴 가깝게(210도 이상) 돌면 매듭(루프)으로 보고 제외
        if(Math.abs(localTurningDeg(poly,iStart-1,iEnd+1))>210)return;
        const len=polyLen(poly);
        if(len>base.len*1.9)return;
        routes.push({poly,len});
      });
    });
  });
  ...
  return uniq; // 유턴·매듭 없는 후보 전부 반환 — 최종 화면 노출은 점수 상위 2~3개만
}
