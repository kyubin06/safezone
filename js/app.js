...
function charBubbleR(gauge){return 26+gauge/100*22;} // 초록 안심원 반지름(게이지 상승 시 성장)
function charIcon(gauge){const S=104,r=charBubbleR(gauge); // 컨테이너는 고정 → 걷기 애니메이션이 리셋되지 않음
  return L.divIcon({className:'',iconSize:[S,S],iconAnchor:[S/2,S/2],
    html:`<div class="char-ic" style="width:${S}px;height:${S}px">
...
function fireEvent(ev){const w=state.walk,m=EVMETA[ev.type];const gained=m.pt*ev.count;
  ...
  if(charMarker){const el=charMarker.getElement();const bb=el&&el.querySelector('.bubble');
    if(bb){const r=charBubbleR(w.gauge);bb.style.width=bb.style.height=(r*2)+'px';}}
